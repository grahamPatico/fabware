...
## Phase 18 scope (shipped)

Phase 18 extends `ExternalPartRef` with an optional `stepUrl` field and updates `compileAssembly` to emit a build123d Python import script for external parts that carry a STEP URL.

### `ExternalPartRef.stepUrl` field (Task 1)

`ExternalPartRef` gains an optional `stepUrl?: string` field in `ir/types.ts`. Zod schema updated in `ir/schema.ts`: `stepUrl: z.string().url().optional()`. Rejects non-URL strings at parse time.

### `compileAssembly` — `import_step` codegen (Task 2)

`compileAssembly(ir)` now handles three cases for `ir.parts` entries:

| Part kind | `stepUrl` present? | Output |
|---|---|---|
| `inline` | n/a | build123d Python script (unchanged) |
| `external` | yes | `import_step` Python script (new in Phase 18) |
| `external` | no | skipped (BOM-only, unchanged) |

The generated Python for an external part with a STEP URL:

```python
# generated by compileAssembly — external part import for <id>
from build123d import *
imported = import_step("/in/external/<vendor>__<partNumber>.step")
positioned = imported.moved(Location((<x>, <y>, <z>), (<rx_rad>, <ry_rad>, <rz_rad>)))
export_step(positioned, "/out/<id>.step")
export_stl(positioned, "/out/<id>.stl")
```

- Sandbox convention: `/in/external/<vendor>__<partNumber>.step` — spaces and slashes in vendor/partNumber are replaced with underscores (`sanitizeForFilename`).
- Origin and rotation come from the Phase 9 `origin`/`rotation` fields. Both default to zero when absent.
- Rotation degrees → radians (`× Math.PI / 180`) for build123d's `Location()`.
- Phase 18 does not fetch STEP files — the codegen only emits the expected path; the sandbox operator supplies the file.

### Tools / prompts / README (Task 3)

- `patch/tools.ts` `add_part` external variant schema: `stepUrl` property added (type string, format uri, optional, with description).
- `patch/__tests__/tools.test.ts`: new assertion verifying `stepUrl` appears in `add_part` schema.
- `prompts.ts`: "External (purchased) parts" section updated with Phase 18 `stepUrl` documentation.
- README: Phase 18 scope section added.

### Tests added

| File | New tests |
|---|---|
| `ir/__tests__/partRefSchema.test.ts` | 2 (accepts valid stepUrl, rejects malformed stepUrl) |
| `codegen/__tests__/compileAssembly.test.ts` | 3 (external+url produces import_step script, external-no-url skipped, inline+external together) |
| `patch/__tests__/tools.test.ts` | 1 (add_part schema includes stepUrl) |

Tests: **≥ 417 passing** (`pnpm test --run`).
