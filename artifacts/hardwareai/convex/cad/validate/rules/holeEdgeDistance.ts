// convex/cad/validate/rules/holeEdgeDistance.ts
//
// Rule: mfg.hole-edge-distance
// Hole centre must be at least (1.5 × d + d/2) mm from every edge of the
// parent extrude's bounding rectangle.
...
/**
 * Returns violations for holes whose centres are too close to the parent
 * extrude's bounding-rectangle edges.
 */
export function holeEdgeDistance(ir: ResolvedIr): Violation[] {
  const out: Violation[] = [];
